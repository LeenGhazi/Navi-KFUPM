...
export function AdminAnnouncementsPage() {
    {/* This page allows maintenance staff to manage campus announcements. */}
    const { user } = useAuth();
    ...
    <div className="container mx-auto px-4 py-8">
      {/* Header section with title, description, and add announcement button. */}
      ...
      {/* List of announcements displayed as cards. */}
      <div className="grid grid-cols-1 gap-4">
        {announcements.map((announcement) => (
          <Card key={announcement._id} className="hover:shadow-md transition-shadow">
            <CardHeader>
              <div className="flex items-start justify-between">
                {/* Left side of the card header with announcement title and priority. */}
            ...
            <CardContent>
              {/* Announcement content and author information. */}
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        {/* This dialog contains a form for adding a new announcement. */}
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4">
            {/* Form fields for announcement title, content, and priority. */}
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                {['Low', 'Medium', 'High'].map((priority) => (
                  <Button key={priority} type="button" variant={formData.priority === priority ? 'default' : 'outline'} onClick={() => setFormData({ ...formData, priority })} className="flex-1">
                    {priority}
                  </Button>
                ))}
              </div>
            ...
            <div className="flex gap-2 pt-4">
              {/* Action buttons for publishing the announcement or canceling. */}
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        {/* This dialog is similar to the add dialog but is pre-filled with the selected announcement's data for editing. */}
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 pt-4">
              {/* Action buttons for saving changes or canceling the edit. */}
              <Button onClick={handleEditAnnouncement} className="flex-1">Save Changes</Button>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        {/* This dialog confirms if the user really wants to delete the selected announcement. */}
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          {selectedAnnouncement && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button onClick={handleDeleteAnnouncement} variant="destructive" className="flex-1">Delete</Button>
                <Button variant="outline" onClick={() => {
                  setShowDeleteDialog(false);
                  setSelectedAnnouncement(null);
                }} className="flex-1">
