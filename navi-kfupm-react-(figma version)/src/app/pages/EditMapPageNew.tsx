...
export function EditMapPage() {
  ...
  const handleSaveChanges = () => {
    ...
    setLocations(prev =>
      prev.map(loc => (loc.id === editForm.id ? { ...loc, ...editForm } as Location : loc))
    );

    setSelectedLocation(editForm as Location);
    toast.success('Building information updated successfully!');
    setIsEditing(false);
  };

  const handleUpdateField = (field: keyof Location, value: any) => {
  ...
  const renderBuildingCard = () => {
    ...
    const displayLocation = (isEditing ? editForm : selectedLocation) as Location;
      ...
      <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              {isEditing ? (
                <Input
                  ...
                  className="font-semibold text-lg h-auto px-2 py-1"
                />
              ) : (
                <span className="font-semibold text-lg">{displayLocation.name}</span>
              )}
            </div>
            {isEditing ? (
              <Select
                value={editForm.category}
                onValueChange={(value) => handleUpdateField('category', value as LocationCategory)}
              >
        ...
        <div>
          {isEditing ? (
            ...
            <p className="text-sm text-muted-foreground">{displayLocation.description}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {(displayLocation.openHours || isEditing) && (
              ...
              <div className="text-sm flex-1">
                ...
                {isEditing ? (
                  <Input
                    ...
                    onChange={(e) => handleUpdateField('openHours', e.target.value)}
          ...
          {(displayLocation.capacity || isEditing) && (
            <div className="flex items-center gap-2">
