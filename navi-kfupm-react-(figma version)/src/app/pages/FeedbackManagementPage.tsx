...
interface Feedback {
  ...
  title: string;
  description: string;
  category: 'Performance' | 'Data Accuracy' | 'Technical Issue' | 'Feature Request' | 'Other';
...
export function FeedbackManagementPage() {
  ...
  const getStatusBadge = (status: Feedback['status']) => {
    const variants = {
      ...
      'In Progress': { variant: 'outline' as const, className: 'bg-blue-50 text-blue-700 border-blue-200' },
      'Resolved': { variant: 'outline' as const, className: 'bg-green-50 text-green-700 border-green-200' },
    };
  ...
  const filterFeedbackByStatus = (status?: Feedback['status']) => {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <Tabs defaultValue="all" className="space-y-6">
        ...
        <TabsContent value="pending" className="space-y-4">
          {filterFeedbackByStatus('Pending').map(feedback => (
            <FeedbackCard key={feedback.id} feedback={feedback} />
          ))}
        </TabsContent>

        <TabsContent value="inProgress" className="space-y-4">
          {filterFeedbackByStatus('In Progress').map(feedback => (
            <FeedbackCard key={feedback.id} feedback={feedback} />
          ))}
        </TabsContent>

        <TabsContent value="resolved" className="space-y-4">
          {filterFeedbackByStatus('Resolved').map(feedback => (
            <FeedbackCard key={feedback.id} feedback={feedback} />
          ))}
        </TabsContent>
      </Tabs>
