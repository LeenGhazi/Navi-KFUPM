...
export function AdminCommunityPathsReview() {
  const { user } = useAuth();
  const [paths, setPaths] = useState<CommunityPath[]>(mockPaths);
  ...
  const handleApprove = (pathId: string) => {
    if (!reviewNotes.trim() && selectedPath?.id === pathId) {
  ...
  const handleReject = (pathId: string) => {
    if (!reviewNotes.trim() && selectedPath?.id === pathId) {
  ...
  const getStatusIcon = (status: CommunityPath['status']) => {
    switch (status) {
      case 'Pending':
  ...
  const getStatusColor = (status: CommunityPath['status']) => {
    switch (status) {
      case 'Pending':
                  ...
                  <div className="space-y-3 pt-2 border-t">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        value={selectedPath?.id === path.id ? reviewNotes : ''}
                        onChange={(e) => {
