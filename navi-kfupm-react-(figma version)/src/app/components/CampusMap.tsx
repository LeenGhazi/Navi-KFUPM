...
import { Location, BusRoute, MainPath } from '@/types';
...
interface CampusMapProps {
  ...
  showMainPaths: boolean;
  searchQuery: string;
  onLocationClick: (location: Location) => void;
  routeFrom: string | null;
  routeTo: string | null;
  showMultipleRoutes: boolean;
  movingBuildingId?: string | null;
  onBuildingMoved?: () => void;
}
...
export function CampusMap({
  ...
  const generateRoutes = (from: { x: number; y: number }, to: { x: number; y: number }) => {
  ...
  return (
    <div className="relative w-full h-full bg-gray-100 dark:bg-gray-900 overflow-hidden rounded-lg border">
      {/* Map Controls */}
      ...
      {calculatedRoutes.length > 0 && (
        <Card className="absolute top-4 left-4 z-10 p-3 max-w-md shadow-lg">
      ...
      >
        <svg
          width="100%"
          height="100%"
          className="absolute inset-0 bg-[#E8EDE7]"
          style={{
        ...
        >
          ...
          {/* Main Paths */}
          {showMainPaths &&
            mockMainPaths.map((path) => (
              <g key={path.id}>
                <polyline
                  points={path.path.map((p) => `${p.x},${p.y}`).join(' ')}
                  fill="none"
                  stroke={path.color}
                  strokeWidth="4"
                  strokeDasharray="8,4"
          ...
          {calculatedRoutes.map((route) => (
            <g key={route.id}>
              <polyline
                ...
                strokeWidth="6"
                opacity="0.8"
                strokeDasharray={route.id === 'direct' ? '0' : '10,5'}
              />
              {route.path.slice(1, -1).map((point, idx) => (
                <circle
                  key={idx}
                  cx={point.x}
                  cy={point.y}
          ...
          {filteredLocations.map((location) => {
            ...
            return (
              <g
                ...
                style={{ cursor: movingBuildingId ? 'not-allowed' : 'pointer' }}
              >
                {/* Building shadow */}
                <rect
                  x={buildingX + 3}
                ...
                <rect
                  ...
                  width={shape.width}
                  height={shape.height}
                  fill={isMoving ? '#3B82F6' : color}
                  stroke={isMoving ? '#FCD34D' : isRoutePoint ? '#FCD34D' : isHovered ? 'white' : '#E5E7EB'}
                  strokeWidth={isMoving ? '5' : isRoutePoint ? '4' : isHovered ? '3' : '2'}
                ...
                <text
                  ...
                  className="fill-white font-bold text-shadow"
                  style={{
                ...
                {isRoutePoint && (
                  <>
                    <circle
                      ...
                      cy={buildingY - 15}
                      r="10"
                      fill={location.id === routeFrom ? '#3B82F6' : '#EF4444'}
                      stroke="white"
                      strokeWidth="2"
                    />
                    <text
