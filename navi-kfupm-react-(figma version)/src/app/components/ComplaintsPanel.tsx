...
export function ComplaintsPanel() {
  ...
  const handleUpdateStatus = (complaintId: string, newStatus: string) => {
    setComplaints((prev) =>
      prev.map((c) =>
        c.id === complaintId
          ? { ...c, status: newStatus as any, notes, updatedAt: new Date().toISOString() }
          : c
      )
    );
  ...
  const ComplaintCard = ({ complaint }: { complaint: Complaint }) => (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {user?.role === 'maintenance_staff' && complaint.status !== 'resolved' && (
          <Button
            size="sm"
  ...
  return (
    <>
      <Card className="h-full">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {user.role === 'student' && (
              <Button onClick={() => setShowAddComplaint(true)} className="gap-2">
          ...
          <Tabs defaultValue="my-complaints">
            ...
            {user.role === 'admin' && (
              <TabsContent value="all">
                <ScrollArea className="h-[400px] pr-4">
                  <div className="space-y-3">
                    {allComplaintsForAdmin.map((complaint) => (
                      <ComplaintCard key={complaint.id} complaint={complaint} />
                    ))}
                  </div>
                </ScrollArea>
      ...
      {selectedComplaint && user.role === 'maintenance_staff' && (
        <Card className="fixed bottom-4 right-4 w-96 z-50 shadow-lg">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                className="flex-1"
                onClick={() => handleUpdateStatus(selectedComplaint.id, statusUpdate)}
              >
                Update
              </Button>
              <Button
      ...
      {showAddComplaint && user.role === 'student' && (
        <Card className="fixed inset-0 m-auto w-[500px] h-fit z-50 shadow-lg">
          <CardHeader>
          ...
          <CardContent>
            <form onSubmit={handleSubmitComplaint} className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  value={newComplaint.locationId}
                  onValueChange={(value) =>
                    setNewComplaint({ ...newComplaint, locationId: value })
                  }
                  required
                >
                  <SelectTrigger>
